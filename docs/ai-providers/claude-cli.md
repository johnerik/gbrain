...
| Cost figures | The recipe declares `cost_per_1m_input_usd: 3.0` / `cost_per_1m_output_usd: 15.0` — the same Sonnet-class figures the `anthropic` recipe declares (`price_last_verified: 2026-06-17`) — purely so gbrain's budget ledger has a number to attribute per call. Neither the recipe nor the adapter code checks what you're actually billed; treat these as the ledger's nominal per-call number, not a verified charge. These recipe-declared figures are display-only (`gbrain providers`) — the `--max-usd` budget gate below does not read them. |
| Cost caps | `--max-usd` / `--max-cost` on a `claude-cli:<alias>` model (e.g. `claude-cli:haiku`) can't be enforced — aliases aren't in the pricing table, so `BudgetTracker.reserve()` warns once (`"has no pricing entry; --max-usd cannot be enforced, proceeding unmetered"`) and runs the work uncapped instead of hard-failing with `no_pricing`. A `claude-cli:<dated-id>` (e.g. `claude-cli:claude-haiku-4-5-20251001`) IS priced — the dated id happens to match a bare `anthropic:` pricing key — so the cap enforces normally there, at the nominal Anthropic per-token rate even though billing actually rides your CLI subscription. To force real enforcement (or a $0 rate) on any `claude-cli:*` string, declare an operator rate: `gbrain config set pricing.overrides '{"claude-cli:haiku": 0}'` (#4312). |
